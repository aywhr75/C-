...
int main(int argc, char** argv) {
	...
	sdds::Collection<sdds::Book> library("Bestsellers");
	{
		...
		do{
			getline(file, str);	// Check if this is a commented line which start with '#
			if (file) if (str[0] != '#') library += Book(str);
		} while (file);
	...
	auto updatePrice = [=](Book & b1) {
		if (b1.country() == "US") {
			b1.price() *= usdToCadRate;
		}

		if (b1.country() == "UK" && b1.year() >= 1990 && b1.year() <= 1999) {
			b1.price() *= gbpToCadRate;
		}
	};
	...
	Movie movies[5];
	{
		...
		do{
			getline(file2, str);
			if (file2){
				// Check if this is a commented line which start with '#
				if (str[0] != '#'){
					movies[index] = Movie(str);
					index++;
				}
			}
		} while (file2);
